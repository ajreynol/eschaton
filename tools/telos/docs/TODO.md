...
| **T2** | a proof-carrying rewriter for one theory | **[I3](design.md#i3--rewrites-prove-themselves-as-they-fire)**, the riskiest inversion, and the only one Logos does not settle | a week | the answer to whether telos's central claim holds |
...
| **the generated-rewriter route** | marking a RARE rule `:exec` on `ajreynol/cvc5` `rdbExec` at `4585967004` | **one token per rule**, with the matcher regenerated by `contrib/install-rare-rewrites` |

The second is the hard one, and it is the point.
[I3](design.md#i3--rewrites-prove-themselves-as-they-fire) sets out what
that route does and does not close. **A result that beats the 2022 objection and
loses to `:exec` is a negative result for telos**, and recording it that way is
